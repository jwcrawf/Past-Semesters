/*
  ...
  cpp file with implementations for the wordsmith3 API
...
bool WordSmith::WriteReport(const fsu::String& outfile, unsigned short kw, unsigned short dw) const
{
  ...
  for (MapType::ConstIterator i = frequency_.Begin(); i != frequency_.End(); ++i)
  {
    outData << std::left  << std::setw(kw) << (*i).key_
            << std::right << std::setw(dw) << (*i).data_ << std::endl;
  }
...
void WordSmith::ShowAnalysis () const
{
